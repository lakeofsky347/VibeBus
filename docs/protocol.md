...
Registration returns both a bearer token and a recovery key in plaintext once by default. CLI `--store-credentials` or MCP `storeCredentials=true` writes the pair to the current user's OS vault and, on success, returns metadata with `secretsRedacted=true` instead of either secret. Windows uses a Generic Credential; macOS uses a Keychain generic password with account `VibeBus`. The service/target is `VibeBus:<project-id>:<agent>` and never enters the repository or SQLite database.
