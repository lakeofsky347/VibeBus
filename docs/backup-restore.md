...
Logical retention remains independent from SQLite file compaction. The current product does not expose `VACUUM`, and the restore drill does not run it. A future compaction command must be designed as a separate offline maintenance operation with, at minimum, a fresh verified backup, a real-terminal exact confirmation, exclusive project downtime, sufficient free-space validation, before/after hashes and sizes, `doctor` verification, an auditable result, and no MCP mutation path. Until that design is implemented and approved, use a verified backup plus offline cutover when physical file replacement is required.
