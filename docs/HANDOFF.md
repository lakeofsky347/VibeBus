...
The fixed two-real-task desktop fixture also has a repository-owned, read-only start-state verifier at `scripts/preflight-desktop-acceptance.ps1`. Run it without `-SkipGit` before creating either user-owned task and require zero failed or skipped checks; it validates the exact pristine task graph, absent disposable identities/credentials/bindings/reservation, schema and Operator state, immutable backup identity, and clean pushed Git state.
...
1. Run `scripts/preflight-desktop-acceptance.ps1` without `-SkipGit`, then perform the two-real-task desktop acceptance in `docs/desktop-acceptance.md`; the deterministic fixture, exact handoff evidence schema, non-destructive preflight/audit scripts, evidence gates, and copy-ready A/B prompts are prepared, but creating user-owned top-level tasks still requires explicit user action/authorization.
