...
## Current G1 production-release truth (2026-07-21)

This section supersedes historical delivery narrative below when they disagree. G1 freezes the only authorized production candidate as a **Windows x64 signed GitHub Release `v0.10.0`**. macOS and Linux remain local-development/container acceptance paths and are excluded from the release decision. The release is currently NO-GO until the draft PR is merged, a matching `v0.10.0` tag exists, the protected release environment exposes auditable Windows signing evidence, and downloaded signed assets pass checksum, Authenticode, isolated install, optional upgrade, uninstall, user `PATH`, marketplace cleanup, and recovery checks.

The source release path is clean/tagged-source only: `plugins/vibebus/bin/vibebus.exe` is generated during the Windows build and must not be tracked. CI/release Actions are immutable-SHA pinned; `cargo-deny` gates RustSec advisories, licenses, banned dependencies, and sources; the tag candidate includes a CycloneDX SBOM and `supply-chain-evidence.json`. The Docker base-image digests, remote linux/amd64 platform, and OCI source-revision label are verified for the separate container path, not treated as a Windows release substitute.

Do not alter repository visibility, secrets, environments, PRs, tags, or GitHub Releases from this task. No source commit or push is authorized here. Treat prior stated public visibility, historical ACR pushes, unsigned packages, and ad-hoc macOS signing only as historical evidence, not as production approval.
