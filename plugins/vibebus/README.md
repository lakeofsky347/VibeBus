...
VibeBus 是独立 Codex 顶层任务之间的结构化本地总线：每个任务继续拥有自己的聊天上下文和 worktree，只通过明确登记的事实协作。插件把协调 Skill、stdio MCP、Windows/macOS 生命周期 Hooks、品牌资产和可安装的本地 marketplace 打包在一起。
...
- Windows Credential Manager/macOS Keychain、token 摘要存储、恢复密钥轮换和脱敏响应。
...
启动新 Codex 任务并审查/信任 Hooks。macOS Hooks 由同一个原生二进制执行，不依赖 PowerShell、Node、jq 或 transcript。Keychain 写入成功时返回 `backend=macos-keychain` 与 `secretsRedacted=true`。
...
- Agent token 和 recovery key 只在注册/恢复/轮换边界出现；SQLite 仅保存摘要，成功写入 Windows Credential Manager 或 macOS Keychain 后响应脱敏。
