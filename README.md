...
VibeBus 是一个面向独立 Codex 顶层任务的本地结构化事实总线。它保留 Codex 原生的任务与 worktree 隔离，只共享明确登记的消息、ACK、任务状态、依赖、已确认决策、文件租约和产物，不共享整段聊天上下文。

当前版本 0.10 是可运行的 Windows MVP：一个 Rust 单文件程序同时提供 CLI 和 stdio MCP，状态写入项目级 SQLite WAL 数据库，Agent 与 operator 秘密可写入 Windows 当前用户凭据管理器，并通过 Windows CI、当前用户级 MSI、便携包及可选本地 Authenticode 签名脚本交付为 Codex 插件。正式 tag 发布缺少签名凭据时会直接失败。
...
- 独立 operator 凭据、仅限真实终端的初始化/轮换/恢复/凭据删除，以及与精确 plan 和当前凭据代次绑定的短时单次 retention 批准。
...
- 项目相对路径租约、任务关联、重叠检测、TTL、所有者续期和显式释放。
- 仓库级责任域策略：角色到 `allowedPaths` 的严格、有界映射；未配置时兼容旧项目，已配置但无效时 fail closed。
- 任务所有者签发的跨责任域授权：认证、任务作用域、TTL、幂等与审计事件齐全。
- 产物登记、项目边界/责任域验证和 SHA-256 校验。
- 不可变 Git commit 与测试结果事实：稳定语义重放、异载荷冲突、最多 200 个变更路径和可选报告产物引用。
- 任务所有者确认的不可变决策、稳定语义 key、产物引用、精确重放与异载荷冲突。
- Agent 级确定性 context sync：仅投影活动 owned task、直接依赖、未读定向消息、相关决策、产物、Git 与测试事实，并支持条目/字节预算和 continuation cursor。
...
- SQLite 健康检查和一致性在线备份。
- CLI、MCP、Codex Skill、SessionStart/PostToolUse/Stop Hooks 和本地 marketplace。PostToolUse 只记录有界事实；Stop 只写待审阅提案，不自动发送 handoff。
...
.\target\release\vibebus.exe responsibility inspect --root D:\path\to\repo --agent api
.\target\release\vibebus.exe reserve add --root D:\path\to\repo --agent api --task TASK-101 --path src/api --reason "TASK-101"
# 仅由任务所有者按需签发短时跨域授权；它不会替代租约。
.\target\release\vibebus.exe responsibility override --root D:\path\to\repo --agent api --task TASK-101 --grantee api --path docs/** --reason "Document TASK-101" --ttl 3600 --idempotency-key TASK-101-docs
...
.\target\release\vibebus.exe decision confirm --root D:\path\to\repo --agent api --task TASK-101 --key api.contract --summary "API contract is confirmed" --idempotency-key TASK-101-api-contract
$context = .\target\release\vibebus.exe context sync --root D:\path\to\repo --agent api --item-limit 100 --byte-budget 65536 | ConvertFrom-Json
# 仅当 hasMore=true 时，把 nextCursor 原样传给下一页；需要最新并发状态时从无 cursor 的第一页重新开始。
.\target\release\vibebus.exe handoff propose --root D:\path\to\repo --agent api --task TASK-101 --item-limit 10
# propose 只生成有界事实视图；审阅后仍需显式 handoff send。
...
# 仅在明确停用 operator 或清理一次性项目时，由维护者在真实终端删除 OS 凭据条目。
.\target\release\vibebus.exe operator delete-credential --root D:\path\to\repo
```
...
安装或更新后启动一个新任务，使 Skill、MCP 和 Hook 重新加载。Hooks 首次使用或内容变更后需要在 Codex 中重新审查并信任。SessionStart 只向上寻找并读取 `.vibebus/project.json`；PostToolUse 不读取 transcript、diff 或测试日志；Stop 只在插件数据目录保存有界提案，不自动投递消息。
...
- operator 秘密使用独立目标 `VibeBusOperator:<project-id>`；数据库只保存摘要与代次。operator 初始化、轮换、凭据恢复、凭据删除和批准只允许真实交互式 CLI，不向 MCP 暴露。删除只移除 OS 凭据条目，不删除数据库中的摘要；随后 `operator status` 会保持 `configured=true` 但报告 `ready=false`。
...
- 已配置责任域策略时，租约、任务产物声明和 Git 变更路径必须命中角色规则或任务作用域内尚未过期的显式授权；授权不会绕过任务所有权、认证或租约冲突。
- 产物路径 canonicalize 后必须仍在项目根目录内。
- Git/test Hook 失败只产生可见降级提示，不能撤销已完成的工具副作用；Hook 不保存原始 diff、日志或完整对话。
- 备份拒绝覆盖已有目标文件。
