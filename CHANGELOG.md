...
- Added CLI-only interactive operator initialization, rotation, vault restoration, status, and retention-plan approval; redirected input, MCP, and normal automation cannot perform operator mutations.
...
- Added migration, expiration, rotation invalidation, concurrent single-consumption, vault redaction/restoration, noninteractive rejection, CLI, and MCP boundary coverage.
